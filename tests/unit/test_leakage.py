...
from leadguard.evaluation.metrics import geographic_split
...
def _make_dummy_data():
    return pd.DataFrame({
        ...
        "service_line_material": ["Lead", "Copper", "Lead", "Galvanized", "Lead", "Copper", "Unknown", "Lead", "Lead", "Galvanized"],
...
def test_test_labels_cannot_affect_test_features():
    ...
    # Baseline features
    test_features_baseline = build_features(test_df, reference_df=train_df, include_label_dependent=True)
    
    # Now randomize test labels
    test_df_randomized = test_df.copy()
    test_df_randomized["service_line_material"] = "Lead" # All Lead!
    
    test_features_randomized = build_features(test_df_randomized, reference_df=train_df, include_label_dependent=True)
...
def test_spatial_features_use_training_labels_only():
    ...
    train_features_baseline = build_features(train_df, reference_df=train_df, include_label_dependent=True)
    test_features_baseline = build_features(test_df, reference_df=train_df, include_label_dependent=True)
    ...
    train_features_modified = build_features(train_df, reference_df=train_df, include_label_dependent=True)
    test_features_modified = build_features(test_df_modified, reference_df=train_df, include_label_dependent=True)
